...
function addProductToCart() {
    ...
    basket.forEach((product) => {
        ...
        <img src="${foundProduct.image1}" alt="${foundProduct.name
            }" style="width:80px; height: 90px;">
        <div class="col">
          <span style="font-weight: bold;">${foundProduct.name}</span> 
          <span style="color:grey;">Quantity: </span>
          <div class="count-btns">
          <button class="btn btn-secondary increase-btn" data-id=${product.id
            }>+</button>
          <span class="count">${product.count}</span>
          <button class="btn btn-secondary decrease-btn" data-id=${product.id
            }>-</button>
          </div>
          <span style="font-weight: bold; font-size: 20px">$${foundProduct.price
            }</span>
          </div>
          <button class="btn btn-danger delete-btn" style="background-color: white; border:none;" data-id=${product.id
            }>X</button>
      </div>`;
